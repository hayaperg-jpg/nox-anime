...
/* IMAGE */
function img(n){
 return 'https://placehold.co/600x850/111827/ff2b88?text='+
...
function details(n){
 ...
 '<h2>'+n+'</h2>'+
 '<p class="muted">Details and episodes</p>'+
 [1,2,3,4,5,6].map(i=>
   '<button onclick="watch('+
   JSON.stringify(n+' — Episode '+i)+
   ')">Episode '+i+'</button>'
 ).join(' ');
...
function watch(n){
 ...
 document.getElementById('body').innerHTML=
 '<h2>▶ '+n+'</h2>'+
 '<div style="aspect-ratio:16/9;background:#050507;display:grid;place-items:center;border-radius:10px;color:white;text-align:center;padding:20px">'+
 'Licensed video source goes here'+
 '</div>'+
...
function adminPanel(){
 ...
 '<button class="pink" onclick="addAnime()">Add</button>'+
 '</div>'+

 A.map((x,i)=>
   '<div class="adminrow">'+
   '<input value="'+x[0]+'" id="n'+i+'">'+
   '<input value="'+x[1]+'" id="g'+i+'">'+
   '<input value="'+x[2]+'" id="r'+i+'">'+
   '<button onclick="delAnime('+i+')">Delete</button>'+
   '</div>'
 ).join('');
